...
| FIX-14 MD did not start bound | DISCOVERED while computing ligand RMSD: `system.gro` places the ligand 26-37 A from the protein (frame/numbering mismatch between docked `.mol2` and MD protein). | DONE (reframed as unbiased association runs; ligand RMSD referenced to each ligand's own bound pose; export + dashboard text corrected) |
...
## FIX-14 result detail (MD setup discovery)
- While computing a real ligand RMSD, the trajectories were found NOT to start from the docked pose: the MD starting structure (`system.gro`) places the ligand 26-37 Angstrom from the protein. Root cause: the system was built by combining a protein and a docked ligand that were in different coordinate frames (the docked `.mol2` is in the original 5CCV crystal frame near the docking grid centre at about -119, 61, 40; the MD protein was renumbered 1-849 and recentred near -54, 37, 25), so the ligand was never merged into the pocket.
- Consequence: these are unbiased association runs, not docked-pose-stability runs. Reanalysis (`scripts/md_reanalyze.py`): celecoxib associates at ~3 ns and stays (ligand RMSD vs its mean bound pose 4.9 +/- 2.1 A); methotrexate associates at ~14 ns and is mobile (4.4 +/- 4.9 A); dasabuvir never forms a stable bound pose (ligand RMSD undefined). Protein backbone RMSD remains physical (2.8-3.4 A).
- Ligand RMSD vs the docked pose was deliberately NOT computed: the docked ligand and the MD protein live in different coordinate frames with inconsistent residue numbering, so bridging them would need error-prone cross-structure sequence alignment that could introduce new artifacts (violating ground rule 1). RMSD is therefore referenced to each ligand's own equilibrated bound pose, and left undefined when there is none. The ligand is made whole and placed in its true minimum-image position (triclinic-aware, anchored to the nearest protein atom) before alignment.

## Completion (2026-06-02)
All 13 original fixes plus the re-dock are DONE, and FIX-14 (the MD association discovery) is documented and reflected in the outputs. 265 pytest tests pass. The professor-facing PDF (`~/Downloads/GeneTropica_Dashboard_Data_Export_<date>.pdf`) now surfaces: per-target validation badges, predicted-contact labelling with the detection method and PDB numbering note, the per-target docking grid, per-link literature evidence tiers with PMID verification, and an honest MD section (unbiased association with a real ligand RMSD vs the bound pose). Headline scientific results: removing molecular-size bias dropped the oversized HCV direct-acting antivirals out of the NS5 top ranks (dasabuvir is now the docking #1); the NS5 AUC 0.37 honest-failure result is preserved; and in unbiased MD the docking #1 (dasabuvir) does not stably bind, while celecoxib and methotrexate spontaneously associate.
