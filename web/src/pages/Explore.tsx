  ...
  return (
    <div className="wrap" style={{ padding: '56px 0' }}>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center', marginBottom: 16, marginTop: 36 }}>
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder={t('Search drug...')} style={inputStyle} />
      ...
      {field.isLoading ? (
        <p className="mono">{t('Loading candidates...')}</p>
      ) : (
        <DrugTable points={filtered} onSelect={(p) => setSel(p.name)} selected={sel ?? undefined} />
      )}

      <div style={{ marginTop: 52 }}>
        <div className="eyebrow">{t('Drug detail')}</div>
        <label htmlFor="drugsel" style={{ display: 'block', fontFamily: 'var(--mono)', fontSize: 10, letterSpacing: '.1em', textTransform: 'uppercase', color: 'var(--ink-faint)', margin: '12px 0 6px' }}>{t('Select a drug to view details')}</label>
