...
function LiteratureEvidence() {
  ...
  return (
    <div style={{ marginTop: 30 }}>
      <h3 style={{ fontSize: 22 }}>{t('Literature evidence')}</h3>
      <p style={{ fontSize: 15, color: 'var(--ink-soft)', lineHeight: 1.7, margin: '8px 0 14px', maxWidth: 760 }}>
        {t('PubMed references per drug for a target, found by keyword mining (so weak hits dominate and count only as a hint, not proof). Drugs that score well here yet have no prior literature are the more interesting, genuinely novel, leads.')}
      </p>
      ...
      <div className="rstack" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 28 }}>
        <div>
          ...
          )) : <p style={{ fontSize: 13, color: 'var(--ink-faint)' }}>{t('No references for this target.')}</p>}
        </div>
        <div>
          <h4 style={{ fontSize: 14, marginBottom: 10 }}>{t('Novel-discovery candidates')}</h4>
          <p style={{ fontSize: 11.5, color: 'var(--ink-faint)', margin: '0 0 8px' }}>{t('Drug-like, top binders here, with no prior literature.')}</p>
          ...
          )) : <p style={{ fontSize: 13, color: 'var(--ink-faint)' }}>{t('None.')}</p>}
        </div>
      </div>
    </div>
  )
...
export default function Insights() {
  ...
  return (
    <div className="wrap" style={{ padding: '56px 0' }}>
      ...
      <Block title={t('What the model is')}>
        {t('A scikit-learn RandomForest trained on 166 compounds from ChEMBL with measured RdRp activity, using a 2048-bit Morgan fingerprint plus the normalised Vina score as features. Its cross-validated AUC is 0.875. (An earlier version of the dashboard mislabelled this as a GNN; it is a RandomForest.)')}
      </Block>

      <Block title={t('What it is not')}>
        {t('It is')} <b>{t('target-agnostic')}</b>{t(': the same drug receives the same ML score for every target, because the model never sees the specific binding pocket. So it is a prior, a broad activity hint, not a per-target prediction. Treat it as one weak signal among several, not a ranking on its own.')}
      </Block>
      ...
      <Block title={t('Why two metrics, not one')}>
        {t('Raw Vina score favours large molecules; ligand efficiency (grip per atom) favours small ones. Reporting both, over drug-like candidates only, keeps either bias from dominating, which is exactly why the candidate ranking shows both side by side.')}
      </Block>
