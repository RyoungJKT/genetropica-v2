...
export function Steps() {
  return (
    <section style={{ padding: '92px 0' }}>
      <div className="wrap">
        <Reveal>
          <div className="eyebrow">The idea in three steps</div>
          <h2 style={{ fontSize: 'clamp(28px,3.6vw,46px)', marginTop: 14 }}>Reusing medicine we already trust</h2>
          ...
          {STEPS.map((s, i) => (
            <Reveal key={s.n} delay={i * 0.08}>
              <div style={{ position: 'relative', padding: '30px 26px', border: '1px solid var(--line)', borderRadius: 16, background: 'linear-gradient(180deg,var(--paper-2),var(--paper))', overflow: 'hidden', height: '100%' }}>
                ...
                <h3 style={{ fontSize: 23, marginBottom: 10, position: 'relative' }}>{s.h}</h3>
                <p style={{ color: 'var(--ink-soft)', lineHeight: 1.65, fontSize: 15, position: 'relative' }}>{s.p}</p>
                <span style={{ display: 'inline-block', fontFamily: 'var(--mono)', fontSize: 10, letterSpacing: '.08em', textTransform: 'uppercase', color: 'var(--green)', border: '1px solid var(--line)', borderRadius: 100, padding: '4px 10px', marginTop: 16, background: 'var(--paper)' }}>{s.chip}</span>
              </div>
            </Reveal>
          ))}
        </div>
