...
export function AskAssistant() {
  ...
  const ask = async (question: string) => {
    ...
    try {
      ...
      setErr('Could not reach the assistant. It runs on the deployed site, which needs the serverless function and an API key.')
    } finally {
      setLoading(false)
    }
  }
        ...
        <div style={{ position: 'fixed', right: 22, bottom: 74, zIndex: 80, width: 'min(390px, 92vw)', background: 'var(--paper)', border: '2px solid var(--ink)', borderRadius: 16, boxShadow: '0 20px 54px rgba(28,26,23,.32)', padding: 18 }}>
          ...
          <form onSubmit={(e) => { e.preventDefault(); ask(q) }} style={{ display: 'flex', gap: 8 }}>
            <input
              ...
              placeholder="e.g. best NS5 binders?"
              maxLength={500}
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 10 }}>
            {SUGGESTIONS.map((s) => (
              <button key={s} onClick={() => ask(s)} style={{ fontSize: 11, color: 'var(--ink-soft)', background: 'var(--paper-2)', border: '1px solid var(--line)', borderRadius: 100, padding: '5px 10px', cursor: 'pointer', textAlign: 'left' }}>{s}</button>
            ))}
          </div>
          {(answer || err || loading) && (
